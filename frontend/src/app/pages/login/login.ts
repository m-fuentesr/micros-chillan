  ...
  template: `
    <div class="min-h-screen flex flex-col lg:flex-row w-full bg-base-200 lg:bg-base-100 relative">
      ...
      <div class="lg:hidden absolute top-0 left-0 w-full h-[240px] bg-primary rounded-b-[3rem] overflow-hidden z-0">
        ...
        <div class="flex flex-col items-center justify-center h-full pb-3 text-primary-content">
          <div class="w-12 h-12 bg-white text-primary rounded-xl flex items-center justify-center font-black text-xl shadow-lg mb-1">GF</div>
      ...
      <div class="hidden lg:flex w-1/2 bg-primary text-primary-content relative flex-col justify-between p-16 overflow-hidden">
        ...
        <div class="relative z-10">
          ...
          <p class="mt-4 text-lg opacity-90 max-w-sm">
            Acceso exclusivo para personal autorizado. Gestiona usuarios, mantenimiento y reportes en tiempo real.
          </p>
        </div>

        <div class="relative z-10 flex justify-between items-end text-sm opacity-70">
          <div>
            <p>Estado del Sistema: <span class="text-success font-bold">● Operativo</span></p>
      ...
      <div class="w-full lg:w-1/2 flex items-center justify-center p-4 sm:p-8 lg:p-16 z-10">
        <div class="w-full max-w-md bg-base-100 rounded-2xl shadow-xl lg:shadow-none p-6 sm:p-8 mt-[180px] sm:mt-[200px] lg:mt-0 animate-in fade-in slide-in-from-bottom-8 duration-700">
          ...
          <form [formGroup]="loginForm" (ngSubmit)="onSubmit()" class="space-y-5">
            ...
            <button type="submit" class="btn btn-primary btn-block h-12 text-lg shadow-lg shadow-primary/20">
              Ingresar al sistema
            </button>
          </form>
          ...
          <div class="relative my-8">
            ...
            <div class="relative flex justify-center text-xs uppercase tracking-wider font-bold text-base-content/50">
              <span class="px-2 bg-base-100">Accesos directos (Dev)</span>
            </div>
          </div>

          <div class="grid grid-cols-2 gap-3">
            <button type="button" class="btn btn-outline border-base-300 hover:bg-base-200 hover:border-base-300 hover:text-base-content h-12 flex flex-col gap-0 leading-tight py-1" (click)="loginAsAdmin()">
              <span class="font-bold">Admin</span>
              <span class="text-[10px] font-normal opacity-60">Escritorio</span>
            </button>
            <button type="button" class="btn btn-outline border-base-300 hover:bg-base-200 hover:border-base-300 hover:text-base-content h-12 flex flex-col gap-0 leading-tight py-1" (click)="loginAsWorker()">
          ...
          <div class="mt-8 text-center">
            <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-base-200/50 text-xs text-base-content/60">
...
export class Login {
  ...
  constructor() {
    ...
    effect(() => {
      ...
      // Evita navegación redundante si ya estamos en la ruta correcta
  ...
  onSubmit() {
    if (this.loginForm.valid) {
      ...
      this.auth.loginWithCredentials(email, password);
      return;
    }
  ...
  loginAsAdmin() {
    this.auth.loginAsAdminMock();
  }

  loginAsWorker() {
    this.auth.loginAsWorkerMock();
  }
}
